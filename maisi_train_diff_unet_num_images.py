...
def parse_arguments():
    ...
    parser.add_argument(
        "--num-generated-images", "-n",
        type=int,
        default=1,
        help="Number of images to generate during inference. "
             "Each image will have a unique random seed. "
             "More images = longer inference time but more diverse samples."
    )
...
print(f"• Images to generate: {args.num_generated_images}")
...
print("\n🖼️  IMAGE GENERATION:")
if args.num_generated_images == 1:
    print(f"• Generating {args.num_generated_images} image (default)")
    print("  💡 Increase with --num-generated-images for more diverse samples")
else:
    print(f"• Generating {args.num_generated_images} images")
    print(f"  ⏱️  Estimated inference time: ~{args.num_generated_images * 2}-{args.num_generated_images * 5} minutes")
    print("  💡 Each image will have a unique random seed for diversity")
...
def run_multiple_image_inference(env_config_filepath, model_config_filepath, model_def_filepath, num_images, num_gpus=1):
    """
    Generate multiple images by running inference multiple times with different random seeds.
    
    Args:
        env_config_filepath: Path to environment configuration
        model_config_filepath: Path to model configuration  
        model_def_filepath: Path to model definition
        num_images: Number of images to generate
        num_gpus: Number of GPUs to use
    """
    print(f"\n🖼️  GENERATING {num_images} IMAGES...")
    print("=" * 60)
    
    for image_idx in range(num_images):
        print(f"\n🎯 Generating image {image_idx + 1}/{num_images}")
        
        # Create a unique random seed for each image
        import random
        unique_seed = random.randint(1000, 99999)
        
        # Read the current model config
        with open(model_config_filepath, "r") as f:
            current_config = json.load(f)
        
        # Update the random seed for this generation
        current_config["diffusion_unet_inference"]["random_seed"] = unique_seed
        
        # Save temporary config with unique seed
        temp_config_path = model_config_filepath.replace(".json", f"_temp_img{image_idx}.json")
        ...
        module_args = [
            ...
            env_config_filepath,
            "--model_config",
            temp_config_path,  # Use temporary config with unique seed
            "--model_def",
            model_def_filepath,
            "--num_gpus",
            str(num_gpus),
        ...
        print(f"✅ Completed image {image_idx + 1}/{num_images}")
...
if args.num_generated_images > 1:
    # Use our custom multi-image generation function
    run_multiple_image_inference(
        env_config_filepath, 
        model_config_filepath, 
        model_def_filepath, 
        args.num_generated_images, 
        num_gpus
    )
else:
    # Generate single image using original method
    module = "scripts.diff_model_infer"
    module_args = [
        "--env_config",
        env_config_filepath,
